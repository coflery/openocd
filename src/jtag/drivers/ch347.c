...
#define DEFAULT_CH347T_PRODUCT_ID 0x55dd // if no product id is set use this CH347T default
#define DEFAULT_CH347F_PRODUCT_ID 0x55de // if no product id is set use this CH347F default
#define DEFAULT_OTHER_PRODUCT_ID 0x55e7	 // other product id found in https://github.com/WCHSoftGroup/ch347
...
struct ch347_info {
	...
	bool is_ch347f_chip; // true if we found the CH347F chip
...
static int ch347_scratchpad_add_move_path(struct pathmove_command *cmd)
{
	...
	ch347_cmd_start_next(CH347_CMD_JTAG_BIT_OP);
	for (unsigned  i = 0; i < cmd->num_states; i++) {
 ...
 */
static void ch347_scratchpad_add_move_state(enum tap_state state, int skip)
{
	uint8_t tms_scan;
	int tms_len;
 ...
 */
static void ch347_scratchpad_add_run_test(int cycles, enum tap_state state)
{
...
static int ch347_open_device(void)
{
	...
	// if we can claim interface 2 we found a CH347T chip; if we can claim interface 4 we found CH347F chip
	...
	if (retval != ERROR_OK)	{
		...
		ch347.is_ch347f_chip = true;
	}	else {
		ch347.is_ch347f_chip = false;
	}
	...
	if (!ch347.is_ch347f_chip && ch347_device_descriptor.bcdDevice < BYTEWISE_MODE_VERSION) {
